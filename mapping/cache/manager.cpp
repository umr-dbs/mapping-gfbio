...
std::unique_ptr<GenericRaster> LocalCacheManager::query_raster(const GenericOperator &op,
	...
	// Miss
		...
		Log::debug("Full MISS for query: %s on %s", CacheCommon::qr_to_string(rect).c_str(), op.getSemanticId().c_str());
	...
	// Check index (if we aren't on depth = 0 )
		...
		switch (resp) {
			...
			case WorkerConnection::RESP_QUERY_MISS: {
				Log::debug("Full remote MISS for query: %s on %s.", CacheCommon::qr_to_string(rect).c_str(), op.getSemanticId().c_str());
				throw NoSuchElementException("Cache-Miss.");
				break;
			}
			// Puzzle time

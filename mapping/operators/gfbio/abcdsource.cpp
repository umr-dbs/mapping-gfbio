 ...
 */
class ABCDSource : public GenericOperator {
	public:
		ABCDSource(int sourcecounts[], GenericOperator *sources[], Json::Value &params) : GenericOperator(sourcecounts, sources) {
			assumeSources(0);
...
REGISTER_OPERATOR(ABCDSource, "abcdsource");
 ...
 */
double ABCDSource::parseDouble(const XMLCh* text) const {
	double result;
	try {
 ...
 */
bool ABCDSource::handleGathering(DOMElement& unit) {
 ...
 */
void ABCDSource::setFeatureStringAttribute(const std::string& attribute, const std::string& value) {
 ...
 */
void ABCDSource::setFeatureDoubleAttribute(const std::string& attribute, double value) {
	size_t position = points->start_feature.size() - 2;
	try {
...
void ABCDSource::setGlobalStringAttribute(const std::string& attribute, const std::string& value) {
	points->global_attributes.setTextual(attribute, value);
}


void ABCDSource::setGlobalDoubleAttribute(const std::string& attribute, double value) {
 ...
 */
void ABCDSource::handleXMLAttributes(DOMNode& node, std::string path,
		std::function<void(const std::string&, double)> setDoubleAttribute,
		std::function<void(const std::string&, const std::string&)> setStringAttribute) {
 ...
 */
void ABCDSource::handleUnit(DOMElement& unit) {
	bool newFeature = handleGathering(unit);

	std::function<void(const std::string&, double)> fn = std::bind(&ABCDSource::setFeatureDoubleAttribute, this, std::placeholders::_1, std::placeholders::_2);
	std::function<void(const std::string&, const std::string&)> fn2 = std::bind(&ABCDSource::setFeatureStringAttribute, this, std::placeholders::_1, std::placeholders::_2);
...
void ABCDSource::handleGlobalAttributes(DOMElement& dataSet){
	std::function<void(const std::string&, double)> setDoubleAttributeFn = std::bind(&ABCDSource::setGlobalDoubleAttribute, this, std::placeholders::_1, std::placeholders::_2);
	std::function<void(const std::string&, const std::string&)> setStringAttributeFn = std::bind(&ABCDSource::setGlobalStringAttribute, this, std::placeholders::_1, std::placeholders::_2);
...
void ABCDSource::handleUnits(DOMDocument& doc) {
...
std::unique_ptr<PointCollection> ABCDSource::getPointCollection(const QueryRectangle &rect, QueryProfiler &profiler){

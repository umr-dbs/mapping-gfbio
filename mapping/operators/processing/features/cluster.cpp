...
class ClusterOperator: public GenericOperator {
	public:
		ClusterOperator(int sourcecounts[], GenericOperator *sources[],	Json::Value &params);
...
ClusterOperator::ClusterOperator(int sourcecounts[], GenericOperator *sources[], Json::Value &params) : GenericOperator(sourcecounts, sources) {
	assumeSources(1);
}

ClusterOperator::~ClusterOperator() {
}
REGISTER_OPERATOR(ClusterOperator, "cluster");

#ifndef MAPPING_OPERATOR_STUBS
std::unique_ptr<PointCollection> ClusterOperator::getPointCollection(const QueryRectangle &rect, QueryProfiler &profiler) {

...
#include "../../../../mapping-gfbio/src/util/terminology.h"
...
TEST(Terminology, requestLabel){
  std::string first = "plum";
    ...
    EXPECT_EQ(Terminology::resolveSingle(first, term1, "label", EMPTY), "Prunus domestica");
    EXPECT_EQ(Terminology::resolveSingle(second, term2, "label", EMPTY), "Apis mellifera Linnaeus, 1758");
...
TEST(Terminology, multiplerequests){
    ...
    Terminology::resolveMultiple(names_in, names_out, terminology, "label", EMPTY);
